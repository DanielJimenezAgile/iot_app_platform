    ...
    <div class="row" v-if="$store.state.devices.length > 0">
      <div class="col-sm-12">
        <card>
          ...
          <el-table
            v-if="$store.state.selectedDevice.alarmRule.length > 0"
            :data="$store.state.selectedDevice.alarmRule"
          >
...
export default {
  ...
  methods:{
    createNewRule(){
      if (this.newRule.condition == null || this.newRule.value == null || this.selectedWidgetIndex == null) {
          this.$notify({
                  type:"danger",
    ...
    const toSend = {
     ...
     this.$axios.post('/alarm-rule',toSend,axiosHeader)
        .then(res=>{
    ...
    deleteDevice(rule){
      const axiosHeader = {
       ...
       this.$axios.delete('/alarm-rule', axiosHeader)
        .then(res=>{
           if(res.data.status = "success"){
